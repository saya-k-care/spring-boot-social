...
public class FooController {
	...
	/*
	 * @GetMapping(value = "/askAI") public ResponseEntity<?> askAI() throws
	 * JsonProcessingException {
	 * 
	 * Object obj =
	 * SecurityContextHolder.getContext().getAuthentication().getPrincipal();
	 * 
	 * if (obj instanceof Jwt) { Jwt jwt = (Jwt) obj; // Extract email from the JWT
	 * claims (assuming email is present as a claim) String email =
	 * jwt.getClaimAsString("email"); System.out.println("email from JWT: " +
	 * email);
	 * 
	 ...
	 * 
	 * Learning learning = new Learning(); learning.setQues("test");
	 * 
	 * try { learningService.save(learning); } catch (Exception e) {
	 * System.out.println(e); }
	 * 
	 * 
	 * } else if (obj instanceof OAuth2User) { OAuth2User oauth2User = (OAuth2User)
	 * obj; // Extract email from OAuth2User's attributes String email =
	 * oauth2User.getAttribute("email");
	 * System.out.println("Email from OAuth2User: " + email); } else {
	 ...
	 * 
	 * 
	 * Map<String, Object> response = new HashMap<>(); try { ObjectMapper
	 * objectMapper = new ObjectMapper(); String aString =
	 * objectMapper.writeValueAsString(learningService.toList());
	 * response.put("messages", aString); return new ResponseEntity<>(response,
	 * HttpStatus.OK); } catch (DataAccessException e) { response.put("message",
	 * messageSource.getMessage("error.database", null, null));
	 * response.put("error", e.getMessage() + ": " +
	 * e.getMostSpecificCause().getMessage()); return new ResponseEntity<>(response,
	 ...
	 * 
	 * @GetMapping(value = "/learning") public ResponseEntity<?> getQues() throws
	 * JsonProcessingException {
	 * 
	 * Object obj =
	 * SecurityContextHolder.getContext().getAuthentication().getPrincipal();
	 ...
	 * chatService.chatGeneralFeedback("Anything special today?"));
	 * 
	 * Map<String, Object> response = new HashMap<>(); try { ObjectMapper
	 * objectMapper = new ObjectMapper(); String aString =
	 * objectMapper.writeValueAsString(learningService.toList());
